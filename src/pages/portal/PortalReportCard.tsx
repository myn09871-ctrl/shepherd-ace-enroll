...
const PortalReportCard = () => {
  ...
  const fetchAvailableReports = async () => {
    ...
    const { data } = await supabase
      .from("report_cards").select("academic_year, term")
  ...
  const fetchReportCard = async () => {
    ...
    const [rcRes, gradesRes] = await Promise.all([
      supabase.from("report_cards").select("*")
        .eq("student_id", student.id).eq("academic_year", selectedYear)
        .eq("term", selectedTerm).eq("is_published", true).maybeSingle(),
      supabase.from("grades").select("*, subjects(name)")
        .eq("student_id", student.id).eq("academic_year", selectedYear).eq("term", selectedTerm),
    ]);
    if (rcRes.data) {
      ...
      setReportData({
        ...
        student_id: student.student_id, gender: (student as any).gender || "",
        class_name: rc.class_name, academic_year: rc.academic_year, term: rc.term,
        photo_url: student.photo_url, number_on_roll: rc.number_on_roll,
        position_in_class: rc.position_in_class, learner_average: rc.learner_average,
        class_average: rc.class_average, cumulated_score: rc.cumulated_score,
        max_possible_score: rc.max_possible_score, promoted_to: rc.promoted_to,
        next_term_begins: rc.next_term_begins, attendance_present: rc.attendance_present,
        attendance_total: rc.attendance_total, conduct: rc.conduct, attitude: rc.attitude,
        interest: rc.interest, form_teacher_name: rc.form_teacher_name,
        form_teacher_remark: rc.form_teacher_remark, headteacher_name: rc.headteacher_name,
        headteacher_remark: rc.headteacher_remark, grades,
      });
    } else {
      setReportData(null);
    }
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Report_Card_${selectedYear}_Term${selectedTerm}.pdf`);
    } catch (err) {
      console.error(err);
